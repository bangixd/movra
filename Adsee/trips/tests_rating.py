...
class DriverRatingTest(TestCase):
    ...
    def test_rate_driver(self):
        response = self.api.post(f'/api/trips/{self.trip.id}/rate/', {
            'rating': 4,
            'feedback': 'عالی بود'
        }, format='json')
    ...
    def test_rating_updates_average(self):
        # امتیاز اول
        self.api.post(f'/api/trips/{self.trip.id}/rate/', {'rating': 5}, format='json')
        # سفر دوم
        trip2 = Trip.objects.create(
            driver=self.driver_profile, campaign=self.campaign, vehicle=self.vehicle,
            status=Trip.Status.COMPLETED, start_time=timezone.now(), end_time=timezone.now()
        )
        self.api.post(f'/api/trips/{trip2.id}/rate/', {'rating': 3}, format='json')
        self.driver_profile.refresh_from_db()
    ...
    def test_invalid_rating(self):
        response = self.api.post(f'/api/trips/{self.trip.id}/rate/', {'rating': 6}, format='json')
