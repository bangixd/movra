...
class CampaignAnalysisCSVTest(TestCase):
    def setUp(self):
        ...
        self.driver_profile = DriverProfile.objects.create(user=self.driver_user, full_name='Driver Ali', national_id='0987654321')
    ...
    def test_csv_download_by_campaign_owner(self):
        ...
        response = self.client_api.get(f'/api/campaigns/{self.campaign.id}/analysis/csv/')
    ...
    def test_csv_access_denied_for_other_client(self):
        ...
        response = self.other_client_api.get(f'/api/campaigns/{self.campaign.id}/analysis/csv/')
